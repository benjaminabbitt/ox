...
build-features FEATURES:
    cargo build --release -p ox-app --features {{FEATURES}}

# ============== Flash & Run ==============

# Flash and monitor (ESP32-C3)
run: build
    espflash flash --monitor --log-format defmt target/riscv32imc-unknown-none-elf/release/ox-app

# Flash debug build
run-debug: build-debug
    espflash flash --monitor --log-format defmt target/riscv32imc-unknown-none-elf/debug/ox-app
...
run-s3: build-s3
    espflash flash --monitor --log-format defmt target/xtensa-esp32s3-none-elf/release/ox-app
...
# Run all ox-services tests
test-services:
    cargo test -p ox-services --features nav,vehicle,rc,wifi,compass,gps --target x86_64-unknown-linux-gnu
...
# Run with defmt output via probe-rs
defmt: build
